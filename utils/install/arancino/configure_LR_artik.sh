...
if [ "$#" -ne 4 ]; then
        echo "Usage: ./configure_LR_artik.sh <IOTRONIC_BOARD_ID> <IOTRONIC_BOARD_PASSWORD> <WAMP_URL> <WSTUN_URL>"
        exit
fi
...
systemctl enable lightning-rod 
echo " - Lightning-rod enabled at boot."


echo -e "\nLightning-rod starting..."
systemctl restart lightning-rod